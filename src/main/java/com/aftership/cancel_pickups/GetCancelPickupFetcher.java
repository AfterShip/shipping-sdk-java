...
import com.aftership.model.GetCancelPickupResponse;
import com.aftership.model.CancelPickupV3;

public class GetCancelPickupFetcher extends Fetcher<GetCancelPickupResponse> {
    private final Map<String, String> headerParams= new HashMap<>(8);

    public GetCancelPickupFetcher addHeaderParam(final String name, final String value) {
    ...
    public GetCancelPickupFetcher setId(String id) {
        this.id = id;
        return this;
    }

    @Override
    public GetCancelPickupResponse fetch(AfterShipClient client) throws Exception {
        if (id == null || id.isEmpty()) {
            throw new ApiException(ErrorEnum.BAD_REQUEST.name(), "Invalid request: `id` is invalid");
        }
        String path = String.format("/postmen/v3/cancel-pickups/%s", id);
        Request request = new Request(HttpMethod.GET, path);
        setHeaderParams(request);
        Response response = client.request(request);
        AfterShipResponse<CancelPickupV3> responseData = new Gson().fromJson(response.getContent(), new TypeToken<AfterShipResponse<CancelPickupV3>>() {
            }.getType());
        ...
        GetCancelPickupResponse result = new GetCancelPickupResponse();
