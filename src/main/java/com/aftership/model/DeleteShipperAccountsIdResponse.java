/*
 * This code was auto generated by AfterShip SDK Generator.
 * Do not edit the class manually.
 */
package com.aftership.model;

import com.google.gson.annotations.SerializedName;
import com.aftership.base.Resource;
import java.util.List;
import java.util.Map;

public class DeleteShipperAccountsIdResponse extends Resource {

    private java.util.Map<String, java.util.List<String>> responseHeader;

    private DeleteShipperAccountsIdResponseData data;

    public java.util.Map<String, java.util.List<String>> getResponseHeader() {
        return responseHeader;
    }

    public void setResponseHeader(java.util.Map<String, java.util.List<String>> responseHeader) {
        this.responseHeader = responseHeader;
    }

    public DeleteShipperAccountsIdResponseData getData() {
        return data;
    }

    public void setData(DeleteShipperAccountsIdResponseData data) {
        this.data = data;
    }
}
